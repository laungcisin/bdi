...
type SdtBdiRule struct {
	BdiRuleId           int       //主键
	BdiRuleSetId        int   `form:"bdiRuleSetId"`    //规则集主键

	Conditions           string `form:"conditions"`
	ConditionsExpression string
	Operations           string `form:"operations"`
...
func (this *SdtBdiRule) Add() error {
	...
	var insertSql string =
		" insert into sdt_bdi_rule (bdi_rule_set_id, conditions, conditions_expression, operations, operations_expression, remarks, create_user_id, create_time) " +
		" values (?, ?, ?, ?, ?, ?, ?, ?) "

	//TODO:条件表达式和操作表达式后台未完成，先填空
	_, err = o.Raw(insertSql, this.BdiRuleSetId, this.Conditions, "", this.Operations, "", this.Remarks, 0, time.Now()).Exec()
	if err != nil {
		o.Rollback()
		return err
	}
